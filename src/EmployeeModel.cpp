...
EmployeeModel::EmployeeModel(Account *accountHandler, QObject *parent)
    : QAbstractListModel(parent), m_accountHandler(accountHandler) {
...
QVariant EmployeeModel::data(const QModelIndex &index, int role) const {
    ...
    case ShiftRole:     return emp->getShift();
...
void EmployeeModel::addEmployee(const QString& phone, const QString& name, const QString& dob, const QString& cccd, const QString& shift, const QString& avatar, const QString& cccdFront, const QString& cccdBack) {
...
void EmployeeModel::updateEmployee(int index, const QString& phone, const QString& name, const QString& dob, const QString& cccd, const QString& shift, const QString& avatar, const QString& cccdFront, const QString& cccdBack) {
...
void EmployeeModel::exportCSV(const QString& filePath) {
    ...
    for (Employee* emp : m_employees) {
        file << emp->getPhone().toStdString() << ","
             << emp->getName().toStdString() << ","
             << emp->getDob().toStdString() << ","
             << emp->getCccd().toStdString() << ","
             << emp->getShift().toStdString() << ","
             << emp->getAvatar().toStdString() << ","
             << emp->getCccdFront().toStdString() << ","
             << emp->getCccdBack().toStdString() << "\n";
    }
    file.close();
}