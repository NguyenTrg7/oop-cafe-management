...
Page {
    ...
    function checkDuplicate(id, phone, ignoreId) {
        for (var i = 0; i < allEmployeesModel.count; i++) {
            var emp = allEmployeesModel.get(i).empData;
            if (emp.id !== ignoreId) {
                if (emp.id === id) return "Mã nhân viên (ID) đã tồn tại!";
                if (emp.phone === phone) return "Số điện thoại đã tồn tại!";
            }
        }
        return "";
    }

    // Hàm xác định màu sắc cho ca làm dựa vào giờ
    function getShiftColor(timeStr) {
        if (!timeStr) return { bg: "#ECFDF5", border: "#6EE7B7", text: "#047857" };
        var hour = parseInt(timeStr.split(":")[0]);
        if (hour < 12) {
            return { bg: "#ECFDF5", border: "#6EE7B7", text: "#047857" }; // Sáng (Xanh lá)
        } else if (hour < 17) {
            return { bg: "#FFFBEB", border: "#FDE047", text: "#B45309" }; // Chiều (Vàng cam)
        } else {
            return { bg: "#EFF6FF", border: "#93C5FD", text: "#1D4ED8" }; // Tối (Xanh dương)
        }
    }
                ...
                Rectangle {
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            ComboBox {
                                id: cbEmployee
                                Layout.preferredWidth: 200; model: allEmployeesModel; textRole: "text"; font.pixelSize: 14
                            ...
                            // FIX: Thêm editable: true để gõ số tháng tiện hơn
                            ...
                            Button {
                                ...
                                onClicked: {
                                    if (cbEmployee.currentIndex >= 0 && txtShiftTime.text.trim() !== "") {
                                        var emp = allEmployeesModel.get(cbEmployee.currentIndex).empData
                                        if (typeof coffeeSystem !== "undefined" && coffeeSystem.addShift) {
                                            coffeeSystem.addShift(emp.id, emp.name, emp.phone, selectedDateStr, txtShiftTime.text.trim(), repeatMonths.value)
                                    ...
                                    Button {
                                        ...
                                        onClicked: {
                                            if (typeof coffeeSystem !== "undefined" && coffeeSystem.removeShift) {
                                                // FIX: Truyền thêm model.shiftTime để không xoá nhầm ca khác
                ...
                ColumnLayout {
                    ...
                    GridView {
                        ...
                        delegate: Rectangle {
                            ...
                            // FIX: Thêm MouseArea để Click vào hồ sơ
                            MouseArea {
                                id: mouseArea
                                anchors.fill: parent
                                hoverEnabled: true
                                ...
                                onClicked: {
                                    editEmpError.text = "";
                                    editId.text = model.empData.id;
                                    editId.readOnly = true; // Không cho sửa ID
                                    editName.text = model.empData.name;
                                    editPhone.text = model.empData.phone;
                                    editSalary.text = model.empData.salary.toString();
                                    editEmpDialog.open();
                                }
                            }
                            ...
                            RowLayout {
                                ...
                                ColumnLayout {
                                    Layout.fillWidth: true; spacing: 4
                                    Text { text: model.empData.name; font.bold: true; font.pixelSize: 16; color: "#1E293B" }
                                    Text { text: "ID: " + model.empData.id; font.pixelSize: 13; color: "#64748B" }
                                    Text { text: "📞 " + model.empData.phone; font.pixelSize: 13; color: "#64748B" }
                                    Text { text: "💰 Lương: " + model.empData.salary + "đ/h"; font.pixelSize: 13; color: "#059669"; font.bold: true }
                                }
                            }
                        }
                    }
    ...
    // FIX: DIALOG CHỈNH SỬA / XOÁ NHÂN VIÊN
    Dialog {
        id: editEmpDialog
        width: 440; modal: true; anchors.centerIn: parent
        background: Rectangle { color: "#FFFFFF"; radius: 16; border.color: "#CBD5E1"; border.width: 1 }
